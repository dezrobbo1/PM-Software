...
This is not production-scale certification. The earlier professional-shaped **160 possible / 120 active** challenge has now been projected onto the converged path without changing production limits. It fails first at the retained 64-activity admission boundary. More importantly, a faithful 120-active projection also exposes two missing professional semantics in the current productive Work-Method profile: workface/exclusion groups and protected latest-finish constraints. A diagnostic-only copy with those fields removed generates **64,068** placement alternatives against the retained 20,000 cap and implies **334** lexicographic stages if admitted. See [professional-shape scale classification](docs/professional-shape-scale-classification.md).

The next useful step is therefore **not** to raise the activity limit to 160. First compose workface/exclusion and protected latest-finish semantics into the converged Work-Method/productive-time path on a smaller controlled fixture, then rerun the 160/120 challenge and address the next observed scale barrier. Further lower-order objective evidence and richer irregular/calendar-state cases remain separate focused questions. The pooled-feedback allocator is not the active next question after its bounded inconclusive comparison.
