...
The PR #42 cost decomposition retained that admission profile and compared the then-authoritative 138-stage sequential path with exact bounded mixed-radix batching. Across the small, professional-semantics, 64/48 and controlled ladders, every complete candidate matched on method, mode, placement, objective and physical result. A separate fixed 64-activity model varied only the number of sequential proof stages. On one same-environment 64/48 run, model construction took about 124 ms, sequential proof calls took 19.07 s, and exact batching reduced 138 calls to 11 and end-to-end time by 3.89x. Both stage count and placement density increased proof cost, but raw placement generation remained secondary on the retained anchor. The measured classifier derived **Outcome A: canonical-proof dominated, with a material model-size interaction** from direct cost comparisons. See [the historical cost decomposition](docs/placement-canonical-cost-decomposition.md).
