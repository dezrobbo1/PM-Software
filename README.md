...
That future-only path now has one bounded accepted-history composition. A
reference-selected structure is statused through the existing version-two
accepted-progress model; completed work remains fixed history, an in-progress
package fixes its already-begun method/mode/named resources, and its future work
comes from the separately accepted remaining-processing estimate. Packages with
no accepted execution remain structurally free.

In the retained eight-activity falsification case the reference plan selected
`REMOVE=LIFT`, `RESTORE=CRANE` and finish tick 8. At status tick 4, `LIFT`
had accepted actual `[2,4)` and a revised eight-tick remainder. The recovery
correctly kept the begun `LIFT` method, changed the untouched restore package to
`MANUAL`, and finished at tick 12. A deliberately illegal counterfactual that
removed only the begun-method lock reached tick 11 by switching removal to
`SEGMENTED`, demonstrating why accepted structural history must be hard rather
than tradeable. See
...
That converged headless path has now survived a materially larger bounded falsification case: **64 declared / 48 baseline-active activities**, 8 Work Packages, 4 flexible packages / 16 authorised structures, named and pooled resources, alternate activity modes, accepted history, structural recovery and one rolling status cycle. The baseline joint candidate matched 64 separately solved fixed method/mode controls and finished at tick 19. A trusted C04 outage changed untouched WP-07 to ALTERNATIVE while begun WP-02 remained STANDARD; after promotion and later execution, WP-07 remained hard even though an illegal no-lock counterfactual finished one tick earlier. The baseline candidate used 5,626 placements, 5,832 variables, 3,998 constraints and 138 sequential solver stages, taking about 14.15 s end-to-end in CI. See [converged native scale falsification](docs/converged-native-scale-falsification.md).

This is not production-scale certification. The earlier professional-shaped **160 possible / 120 active** challenge still fails first at the retained 64-activity admission boundary. The two formerly missing professional fields now compose on the future-only productive Work-Method path: shared workfaces reserve the entire `[start, finish)` envelope and active `latest_finish` is a hard limit. A small controlled case agrees with independent Python enumeration. Rerunning the faithful 160/120 projection accepts both fields, generates **64,068** raw placements and retains **64,032** after the hard finish filter, against the unchanged 20,000 cap. See [workface and protected-finish experiment](docs/professional-workface-latest-finish.md) and [historical classification](docs/professional-shape-scale-classification.md).

The next cost decomposition retained that admission profile and compared the 138-stage sequential oracle with exact bounded mixed-radix batching. Across the small, professional-semantics, 64/48 and controlled ladders, every complete candidate matched on method, mode, placement, objective and physical result. A separate fixed 64-activity model varied only the number of sequential proof stages. On one same-environment 64/48 run, model construction took about 124 ms, sequential proof calls took 19.07 s, and exact batching reduced 138 calls to 11 and end-to-end time by 3.89x. Both stage count and placement density increased proof cost, but raw placement generation remained secondary on the retained anchor. The measured classifier derives **Outcome A: canonical-proof dominated, with a material model-size interaction** from direct cost comparisons. The challenger remains experimental; the authoritative scheduler is still sequential. See [placement and canonical proof cost decomposition](docs/placement-canonical-cost-decomposition.md).

The evidence supports a focused next milestone to prove and adopt bounded exact batching in the authoritative path while retaining the sequential oracle. Do not raise the 64-activity or 20,000-placement bounds as part of that work. Further lower-order objective evidence and richer irregular/calendar-state cases remain separate questions. The pooled-feedback allocator is not the active next question after its bounded inconclusive comparison.
