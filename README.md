...
This is not production-scale certification. The earlier professional-shaped **160 possible / 120 active** challenge still fails first at the retained 64-activity admission boundary. The two formerly missing professional fields now compose on the future-only productive Work-Method path: shared workfaces reserve the entire `[start, finish)` envelope and active `latest_finish` is a hard limit. A small controlled case agrees with independent Python enumeration. Rerunning the faithful 160/120 projection now accepts both fields, generates **64,068** raw placements and retains **64,032** after the hard finish filter, against the 20,000 cap; the existing compiler shape implies **334** lexicographic stages if admitted. See [workface and protected-finish experiment](docs/professional-workface-latest-finish.md) and [historical classification](docs/professional-shape-scale-classification.md).

The next focused scale question is how to generate/canonicalise placements within a bounded proof budget; retain the 64-activity admission guard while investigating it. Further lower-order objective evidence and richer irregular/calendar-state cases remain separate focused questions. The pooled-feedback allocator is not the active next question after its bounded inconclusive comparison.
