...
The next cost decomposition retained that admission profile and compared the 138-stage sequential oracle with exact bounded mixed-radix batching. Across the small, professional-semantics, 64/48 and two controlled ladders, every method, mode, placement, objective and physical result matched. On one same-environment 64/48 run, model construction took about 124 ms, sequential proof calls took 19.07 s, and exact batching reduced 138 calls to 11 and end-to-end time by 3.89x. Both stage count and placement density increased proof cost, but raw placement generation remained secondary on the retained anchor. **Outcome A: canonical-proof dominated, with a material model-size interaction.** The challenger remains experimental; the authoritative scheduler is still sequential. See [placement and canonical proof cost decomposition](docs/placement-canonical-cost-decomposition.md).
