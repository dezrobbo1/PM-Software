...
5. For a statused recovery, establish the relative **Status point** explicitly. Draft, review and accept one `NOT_STARTED`, `IN_PROGRESS` or `COMPLETED` assertion for every activity. Missing state remains `UNKNOWN` and blocks calculation.
6. For begun work, record actual start, productive periods, mode, applicable named assignments and a separate remaining-work estimate. Group history is displayed as group plus quantity. Corrections explicitly supersede an accepted assertion while retaining the old provenance.
7. Choose **Calculate plan / recovery**. The Python scheduler fixes accepted history and returns only permissible future work; the independent history and future checks must pass before a proposal is displayed.
8. Inspect the plan table and read-only Gantt. Statused results label dark-blue **ACTUAL** and green **FORECAST** periods separately. A visible gap across lunch or overnight is suspension, not resource occupancy.
9. Enter an approval actor and choose **Approve displayed proposal**. Accepted facts stale an old approval but never approve a recovery automatically.
10. Report an availability interval with resource, relative start/finish, reporter and reason. A reported item is not trusted scheduling input. Accept it separately with an acceptance actor, calculate a recovery, compare it with the retained approval, then approve the recovery separately if appropriate.
11. Choose **Save workspace** to download the complete native JSON. **Open workspace** restores version 0, 1 or 2 without recalculation. Inputs, reports, accepted execution provenance, remaining estimates, current approval, pending proposal and earlier approvals round-trip intact.
...
This is a bounded practitioner trial, not a production application. The local launch has an in-memory single-process session store; both transports use local actor strings rather than authenticated identities, repeated daily calendars, capacity-one named resources, disjoint interchangeable groups, finish-to-start precedence, at most 16 authorised mode combinations, and 8–15 activities in the browser profile. It has no drag-to-edit actuals, general preemption or crew handover, percent-complete authority, structural Work–Method subgraphs, adaptive-neighbourhood integration, external schedule import/export, database, collaboration, enterprise permissions, touch-specific interaction design or general workforce/calendar language. Timed partial-group outages remain unsupported. The hosted Preview removes the installation requirement but does not remove those domain limitations.
