...
The adapter accepts only the existing bounded actions: load example, new, open, apply project, invalidate proposal, calculate, approve, report, accept and export. It applies the existing 8–15 activity, capacity-one resource, calendar and authorised-mode bounds. Each portable workspace remains limited to 2 MiB. Hosted requests are limited to 4 MiB so Open can carry both the current browser workspace and a separately selected valid workspace without making either workspace's individual bound larger. Hosted Save returns the authoritative workspace once in `state`; the browser formats that returned value for download instead of duplicating a near-limit workspace in the Function response. Mutations require the exact serving HTTPS Origin and Vercel-forwarded HTTPS scheme. There is no wildcard CORS or public API contract.
