...
def _write_durable_posix(path: Path, data: bytes) -> None:
...
def _write_durable_windows(path: Path, data: bytes) -> None:
    """Create a new file with Windows write-through semantics and flush it."""
    ...
    # FILE_FLAG_WRITE_THROUGH causes NTFS to flush data and metadata generated by
    # the request. FlushFileBuffers then completes the file-handle durability
    # boundary without relying on the POSIX-only directory-fsync pattern.
    handle = create_file(
        str(path),
        generic_write,
        0,
    ...
    try:
        ...
        while offset < len(data):
            chunk = data[offset : offset + 16 * 1024 * 1024]
            buffer = ctypes.create_string_buffer(chunk, len(chunk))
            written = wintypes.DWORD(0)
            if not write_file(
                handle,
                ctypes.cast(buffer, wintypes.LPCVOID),
                len(chunk),
            ...
            if written.value != len(chunk):
                raise OSError(
                    f"short Windows write: expected {len(chunk)}, wrote {written.value}"
                )
...
def _write_durable_normalized_observation(
    ...
    *,
    ...
    try:
        if os.name == "nt":
            _write_durable_windows(path, data)
        else:
            _write_durable_posix(path, data)
